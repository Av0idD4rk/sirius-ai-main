...
const RecommendationCard: React.FC<RecommendationCardProps> = ({ book, isFirst }) => {
    ...
    return (
        <button onClick={onClick} className={`flex flex-col items-center mt-5 bg-white rounded-3xl h-[150px] w-[150px] max-sm:pt-3 max-sm:mt-1.5`}>
                    ...
                    <div className="flex gap-5 self-stretch px-4 py-3 mt-3 text-xs text-center text-white bg-black rounded-b-3xl max-sm:mt-1"><span className="max-sm:mx-auto max-sm:-mt-0.5">{book.title}</span>
                    </div>
        </button>
    );
};
